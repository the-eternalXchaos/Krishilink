...
class _ProductCardState extends State<ProductCard>
  ...
  Widget build(BuildContext context) {
    ...
    // Initialize live chat controller only for logged-in users
          ...
          child: Opacity(
            ...
            child: GestureDetector(
              ...
              child: SizedBox(
                ...
                // Remove fixed height to make card expansible
                child: Card(
    ...
    return IntrinsicHeight(
      child: Column(
        ...
        children: [
          ...
          _buildContentSection(theme, isSmallScreen: isSmallScreen),
        ],
      ),
    );
  }
  ...
  Widget _buildWishlistButton(ThemeData theme) {
    final isLoggedIn =
        Get.isRegistered<AuthController>()
            ...
            : Get.put(AuthController()).isLoggedIn;

    // If not logged in, show a static heart that prompts login on tap (no controller instantiation)
    if (!isLoggedIn) {
      return Positioned(
        top: 12,
        right: 12,
    ...
    // Logged-in: use WishlistController with reactive state
    final wishlistController =
        Get.isRegistered<WishlistController>()
            ? Get.find<WishlistController>()
            : Get.put(WishlistController());
  ...
  Widget _buildLiveIndicator() {
    // Live indicator only for logged-in users to avoid guest API calls
    final isLoggedIn =
        Get.isRegistered<AuthController>()
            ? Get.find<AuthController>().isLoggedIn
            : Get.put(AuthController()).isLoggedIn;
  ...
  Widget _buildContentSection(
    ...
    return Flexible(
      child: Padding(
        padding: EdgeInsets.all(isHorizontal ? 16 : 8),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
          children: [
